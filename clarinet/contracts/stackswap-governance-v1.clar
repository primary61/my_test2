...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? .token-vstsw get-balance contract-caller)))
...
(define-public (vote-for (proposal-id uint) (amount uint))
  (let (
    ...
    (unwrap! (contract-call? .token-vstsw transfer amount user (as-contract tx-sender) none) (err ERR_TRANSFER_FAIL))
    (map-set proposals
      proposal-id 
...
(define-public (return-votes-to-member (proposal-id uint) (member principal))
  (let (
    ...
    (as-contract (contract-call? .token-vstsw transfer vote-count tx-sender member none))
  )
)
